...
void Initial_WindowParam(ParamSet *ext_params){
    ...
    gtk_window_set_title(GTK_WINDOW(windowParam),u8"参数设置");
    ...
    //widgets initial
    btn_apply = gtk_button_new_with_label(u8"应用修改");
    btn_cancel = gtk_button_new_with_label(u8"取消修改");
    btn_default = gtk_button_new_with_label(u8"恢复默认");

    checkBox_meida = gtk_check_button_new_with_label(u8"是否中值滤波");
...
//高低
    labe_cannyL = gtk_label_new(u8"Canny低阈值");
    labe_cannyH = gtk_label_new(u8"Canny高阈值");
    labe_cannyCore = gtk_label_new(u8"Canny核尺寸(odd)");
    labe_ZerBgrL = gtk_label_new(u8"边缘背景低阈值");
    labe_ZerBgrH = gtk_label_new(u8"边缘背景高阈值");
    label_drawColor = gtk_label_new(u8"画笔颜色 [0-255]");
    label_neighborSize = gtk_label_new(u8"边缘邻域尺寸(odd)");
    label_VScale = gtk_label_new(u8"垂直缩放尺度");
    label_HScale = gtk_label_new(u8"水平缩放尺度");
