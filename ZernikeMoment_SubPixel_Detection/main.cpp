...
int main(int argc,char *argv[])
{
    ...
    // Initialize GTK before creating any GTK widgets or windows.
    setlocale(LC_ALL, "");
    gtk_init(&argc,&argv);
    ...
    checkbox_useExample = gtk_check_button_new_with_label(u8"使用示例");
    ...
    menuitem_setting = gtk_menu_item_new_with_label(u8"设置");
    ...
    menuitem_params=gtk_menu_item_new_with_label(u8"参数设置");
    ...
    label_pix_mm = gtk_label_new(u8"单位像素对应毫米值");
    ...
    btn_chooseImage = gtk_button_new_with_label(u8"本地图片");
    ...
    btn_openParamPanel = gtk_button_new_with_label(u8"参数设置");
...
void on_btn_chooseImage_clicked(GtkWidget *button, gpointer userdata){
    ImageSelect = gtk_file_selection_new(u8"选择图片");
